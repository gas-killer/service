...
mod gas_killer_executor_tests {
    ...
    use crate::executor::gas_killer::{
        ...
        GasKillerConfig, ExecutionStatus
    };
    use crate::executor::gas_killer::handlers::{MockGasKillerHandler, MockGasPriceOracle};
    use crate::executor::gas_killer::traits::GasKillerExecutorTrait;
    ...
    async fn test_mock_contract_handler() {
        ...
        let supports = handler.supports_interface(
            Address::from([0x11; 20]),
            FixedBytes::from([0x01, 0x02, 0x03, 0x04]),
        ).await.unwrap();
        assert!(supports);
        
        let valid = handler.verify_aggregated_signature(
            Address::from([0x11; 20]),
    ...
    fn test_failed_execution_result() {
        ...
        assert_eq!(result.error_message, Some("Transaction reverted".to_string()));
