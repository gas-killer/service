...
use super::traits::{GasKillerExecutorTrait, GasKillerContractHandler, GasPriceOracle};
use super::types::{
    ExecutionPackage, GasKillerExecutionResult, GasPriceConfig, 
    GasKillerConfig, ExecutionStatus, StateUpdate
};
use crate::executor::core::{ExecutionResult, VerificationExecutor, VerificationData};
...
impl<H: GasKillerContractHandler> GasKillerExecutor<H> {
    ...
    pub async fn new(
        ...
        let signer = config.private_key.parse::<PrivateKeySigner>()
    ...
    fn encode_state_updates(&self, state_updates: &[StateUpdate]) -> (Vec<FixedBytes<32>>, Vec<FixedBytes<32>>) {
...
impl<H: GasKillerContractHandler> GasKillerExecutorTrait for GasKillerExecutor<H> {
    async fn execute_gas_killer_transaction(
        ...
        if !self.verify_gas_killer_interface(package.target_contract).await? {
            return Err(anyhow!("Contract does not support Gas Killer interface"));
        }
        
        // Step 2: Prepare optimized calldata
        let calldata = self.prepare_calldata(
            &package.target_method,
            &package.params,
            &package,
        )?;
        
        // Step 3: Estimate gas with state preloading
        let gas_estimate = self.estimate_gas_with_state(
        ...
        let tx_hash = self.submit_transaction(
            package.target_contract,
            calldata,
            gas_limit,
            gas_price_config,
    ...
    async fn monitor_transaction(
        ...
        self.wait_for_confirmation(tx_hash, self.config.confirmation_timeout).await
    }
...
impl<H: GasKillerContractHandler> VerificationExecutor for GasKillerExecutor<H> {
    async fn execute_verification(
        ...
        Ok(ExecutionResult {
            transaction_hash: format!("0x{}", hex::encode(payload_hash)),
            block_number: None,
            gas_used: None,
            status: Some(true),
