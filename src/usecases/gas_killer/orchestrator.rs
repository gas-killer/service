...
impl GasKillerOrchestrator {
    ...
    pub async fn run(&self) -> Result<(), String> {
        ...
        loop {
            ...
            self.broadcast_validation_request(&task, operators.clone(), round)
                .await?;
            ...
            self.simulate_operator_responses(&task, &operators, hash)
                .await?;
    ...
    async fn simulate_operator_responses(
        ...
        for (i, operator) in operators.iter().take(responding_count).enumerate() {
            ...
            let state_updates = self
                .validator
                .get_state_updates(&task.task_id)
                .await
                .unwrap_or_default();
    ...
    async fn check_quorum(&self, task_id: &[u8; 32]) -> bool {
        ...
        if let Some(resps) = task_responses {
            ...
            if let Some(request) = requests.get(task_id) {
                let required = ((request.operator_set.len() as f64) * self.config.quorum_threshold)
                    .ceil() as usize;
    ...
    async fn aggregate_signatures(&self, task_id: &[u8; 32]) -> Result<ExecutionPackage, String> {
        let responses = self.operator_responses.read().await;
        let task_responses = responses
            .get(task_id)
            .ok_or_else(|| "No responses for task".to_string())?;

        let valid_responses: Vec<_> = task_responses.iter().filter(|r| r.validated).collect();
    ...
    pub async fn handle_byzantine_operators(&self, malicious: Vec<[u8; 64]>) {
        ...
        for operator in malicious {
            ...
            println!("Slashed Byzantine operator: {:02x?}...", &operator[..8]);
        }
    }
