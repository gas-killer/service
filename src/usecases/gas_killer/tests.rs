...
mod tests {
    ...
    fn test_operator_registry() {
        ...
        registry.statuses.insert(operator2, OperatorStatus::Inactive);
    ...
    fn test_optimization_types() {
        ...
        assert_ne!(OptimizationType::StoragePacking as i32, OptimizationType::BatchedUpdates as i32);
        assert_ne!(OptimizationType::BatchedUpdates as i32, OptimizationType::ColdToWarmSlot as i32);
        assert_ne!(OptimizationType::ColdToWarmSlot as i32, OptimizationType::ZeroToNonZero as i32);
        assert_ne!(OptimizationType::ZeroToNonZero as i32, OptimizationType::NonZeroToZero as i32);
    }
    ...
    async fn test_gas_killer_validator() {
        ...
        // Validate task
        let hash = validator.validate_and_return_expected_hash(&task).await.unwrap();
    ...
    async fn test_gas_killer_executor() {
        ...
        let result = executor.execute_verification(package.clone()).await.unwrap();
    ...
    async fn test_orchestrator_config() {
        ...
        assert_eq!(config.aggregation_frequency, std::time::Duration::from_secs(10));
    ...
    async fn test_gas_analysis_caching() {
        ...
        let hash1 = validator.validate_and_return_expected_hash(&task).await.unwrap();
        
        // Second validation should use cache
        let hash2 = validator.validate_and_return_expected_hash(&task).await.unwrap();
    ...
    async fn test_optimization_type_detection() {
        ...
        for i in 0..5 {
            ...
            let _ = validator.validate_and_return_expected_hash(&task).await.unwrap();
    ...
    async fn test_quorum_calculation() {
        let operators = vec![[1u8; 64], [2u8; 64], [3u8; 64]];
        let quorum_threshold = 0.67;
        
        let required = ((operators.len() as f64) * quorum_threshold).ceil() as usize;
        assert_eq!(required, 3);
        
        let operators = vec![[1u8; 64], [2u8; 64], [3u8; 64], [4u8; 64]];
        let required = ((operators.len() as f64) * quorum_threshold).ceil() as usize;
        assert_eq!(required, 3);
        
        let operators = vec![[1u8; 64], [2u8; 64], [3u8; 64], [4u8; 64], [5u8; 64]];
        let required = ((operators.len() as f64) * quorum_threshold).ceil() as usize;
    ...
    async fn test_orchestrator_initialization() {
        ...
        orchestrator.handle_byzantine_operators(byzantine_operators).await;
    }
}