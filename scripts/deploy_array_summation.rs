...
/// legacy wrapper.
fn resolve_avs_address(avs_deployment: &AvsDeploymentJson) -> Result<Address, DynError> {
    match (
...
async fn main() -> Result<(), DynError> {
    ...
    // against (default: ecdsa). The ecdsa path is unchanged; the schnorr path
    // additionally deploys the SchnorrStakeRegistry and registers the operators'
...
async fn deploy_ecdsa() -> Result<(), DynError> {
    ...
    let avs_address = resolve_avs_address(&avs_deployment)?;
...
/// Suffix of the per-operator key files the eigenlayer setup container writes to
/// `<dirname(AVS_DEPLOYMENT_PATH)>/operator_keys/`.
const OPERATOR_KEY_FILE_SUFFIX: &str = ".private.ecdsa.key.json";

#[derive(Debug, Deserialize)]
struct OperatorKeyFile {
    /// The operator's secp256k1 private key. The Schnorr key IS this key — same
    /// scalar, same point, same operator identity address.
    #[serde(rename = "privateKey")]
    private_key: String,
}

/// Loads every operator's Schnorr key from the key files next to the deployment JSON
/// (override the directory with `OPERATOR_KEYS_DIR`), sorted by filename.
...
async fn deploy_schnorr() -> Result<(), DynError> {
    ...
    // The AVS reference still comes from the eigenlayer deployment JSON — operators
    // register with EigenLayer through the ECDSA service manager regardless of the
