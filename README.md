...
- `AVS_REFERENCE_TARGET`, `AVS_REFERENCE_TARGET_FILE`, `DEMO_TARGET_ADDRESS`, `DEMO_TARGET_FILE`, `DEMO_FACTORY_ADDRESS`, `DEMO_FACTORY_FILE`: feed the `contracts` block on `GET /avs-metadata`, which publishes the addresses a target must be wired to in order to settle — `chainId`, `avsAddress`, `blsSignatureChecker`, `registryCoordinator` — plus the demo contracts the docs point at. A target wired to a superseded checker passes every router-side check, is handed a `ready` payload, and then reverts `InvalidQuorumApkHash` on chain, so serving the live pair from the running deployment is what makes that unwireable by accident. `AVS_REFERENCE_TARGET` names the deployed target whose `avsAddress()` and `blsSignatureChecker()` getters establish the pair. `AVS_REFERENCE_TARGET_FILE` instead names a file holding that address, which is what the Helm chart sets when the target deploy job runs (`/app/.nodes/demo_target.txt`): the job rewrites it on every redeployment, so the pair tracks what was actually deployed with nothing to configure. Where neither is set, a `demo_target.txt` beside `avs_deploy.json` is used, and failing that the block is left off. `registryCoordinator` comes from `avs_deploy.json`, and the checker's own `registryCoordinator()` is verified against it — on mismatch the block is omitted rather than serving a pair that reverts. Addresses are published EIP-55 checksummed so they paste straight into Solidity. `DEMO_TARGET_ADDRESS` is a shared playground target and `DEMO_FACTORY_ADDRESS` an `ArraySummationFactory` for a caller-owned instance; both are omitted when unset or when they have no code on the published chain. `DEMO_TARGET_FILE` and `DEMO_FACTORY_FILE` name files holding those addresses instead, which is what the chart sets where the playground job runs, and a recorded playground target is also preferred as the reference target so the published `blsSignatureChecker` is the one `demoTarget.blsSignatureChecker()` returns. Resolution runs off the request path — the endpoint is public and unauthenticated — and publishes the most complete set it can establish, retrying with backoff while any record a deploy job is expected to write is still unread. So a provider blip does not leave the block absent until the next restart, a job that finishes late adds its addresses to what is already being served, and one job failing outright does not withhold the addresses it has nothing to do with.
