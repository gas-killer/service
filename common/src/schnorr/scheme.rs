...
///
/// Participant identity keys are the operators' **BN254 G2 keys** — the p2p transport
/// identity, which `Scheme::PublicKey` must equal because the engine correlates acks to
/// the peer that sent them (`Engine::start` ties the channel's `PublicKey` to this type).
/// The Schnorr signing keys are a separate secp256k1 point per operator, carried
/// index-aligned in `keys`; the caller pairs them (both come off the same operator
/// record). Indices are positions in the ordered BN254 set and MUST match on every
/// process — the router builds the identical set from the same operator state.
///
/// Consequence to keep in mind: participant order is BN254-key order, which is unrelated
/// to identity-address order. Anything the chain wants sorted by address (`nonSigners`)
/// must be sorted explicitly — see [`Self::non_signer_identity_addresses`].
#[derive(Clone)]
pub struct SchnorrScheme {
    /// Ordered BN254 participant identities; participant indices are positions here.
...
impl SchnorrScheme {
    /// Validates the participant/Schnorr-key pairing.
    ///
    /// `schnorr_keys` must already be index-aligned with `participants` (the caller builds
    /// both from one operator record set, exactly as the BLS arm aligns its G1 keys).
    /// Returns `None` on a length mismatch or a duplicate Schnorr key — a duplicate would
    /// make two participants share nonce coverage and an identity address, so
    /// `nonSigners` could no longer name one of them. Panics on an empty set
    /// (construction-time configuration error, mirroring `Bn254Scheme`).
    fn align_keys(
        participants: &Set<OperatorKey>,
        schnorr_keys: Vec<SchnorrPublicKey>,
    ) -> Option<Arc<[SchnorrPublicKey]>> {
        assert!(
            !schnorr_keys.is_empty(),
            "participant set must not be empty (quorum math panics on n == 0)"
        );
        if schnorr_keys.len() != participants.len() {
            return None;
        }
        let distinct: std::collections::HashSet<Address> =
            schnorr_keys.iter().map(|k| k.eth_address()).collect();
        if distinct.len() != schnorr_keys.len() {
            return None;
        }
        Some(schnorr_keys.into())
    }

    /// Creates a signing scheme instance for the operator holding `private_key`.
    ///
    /// `participants` is the ordered BN254 identity set and `schnorr_keys` the
    /// index-aligned Schnorr points. Returns `None` on a malformed pairing (see
    /// [`Self::align_keys`]) or if `private_key`'s public point is not among
    /// `schnorr_keys` — which is also what fixes this operator's participant index, so a
    /// key paired against the wrong slot cannot start.
    pub fn signer(
        participants: Set<OperatorKey>,
        schnorr_keys: Vec<SchnorrPublicKey>,
        ...
        let aligned = Self::align_keys(&participants, schnorr_keys)?;
        let own = private_key.public_key();
        let index = aligned
            .iter()
            .position(|k| *k == own)
    ...
    /// Maps a certificate bitmap to the signers' operator identity addresses, strictly
    /// ascending.
    ///
    /// Sorted explicitly: participant order is BN254-key order, which says nothing about
    /// address order.
    pub fn signer_identity_addresses(&self, signers: &Signers) -> Vec<Address> {
        let mut addresses: Vec<Address> = signers
            .iter()
    ...
    /// subtracts on-chain (it rejects an unsorted list, and `align_keys` rules out the
    /// duplicate addresses that would make "strictly" unachievable).
    pub fn non_signer_identity_addresses(&self, signers: &Signers) -> Vec<Address> {
        ...
        let mut addresses: Vec<Address> = (0..self.participants.len() as u32)
...
pub(crate) mod test_support {
    //! Shared fixture for the scheme and batch-store suites: an operator set whose BN254
    //! identities and Schnorr keys are paired the way the node/router wire them, with every
    //! vector in participant order so index `i` means participant `i` everywhere.

    use super::{OperatorKey, SchnorrPublicKey};
    ...
    pub struct Operators {
        /// Ordered BN254 participant identities.
        pub participants: Set<OperatorKey>,
        /// Schnorr public points, index-aligned with `participants`.
        pub schnorr_keys: Vec<SchnorrPublicKey>,
        /// Schnorr private keys, index-aligned with `participants`.
        pub keys: Vec<PrivateKey>,
        /// BN254 identities, index-aligned with `participants` (what a p2p `sender` is).
        pub identities: Vec<OperatorKey>,
    }

    /// Builds `n` operators seeded deterministically from `seed_base`.
    ///
    ...
    pub fn operators(n: usize, seed_base: u64) -> Operators {
        ...
        let schnorr: Vec<PrivateKey> = (0..n)
            ...
            .collect();
        let participants: Set<OperatorKey> = bn254
            .iter()
            .map(|k| k.public_key())
            .try_collect()
            .expect("distinct BN254 seeds");
        assert_eq!(participants.len(), n, "no duplicate participants");

        let mut keys: Vec<Option<PrivateKey>> = (0..n).map(|_| None).collect();
        for (bn, sk) in bn254.iter().zip(schnorr) {
...
mod tests {
    ...
    /// order — the order of the BN254 identity set, which `test_support::operators`
    /// aligns everything to — so index `i` everywhere means participant `i`.
    fn setup_with(n: usize, uncovered: &[usize]) -> Fixture {
        let ops = test_support::operators(n, 100);
        let keys = ops.keys;
        let pubkeys = ops.schnorr_keys;
        ...
        let schemes: Vec<SchnorrScheme> = keys
            ...
            .map(|(i, key)| {
                ...
                // Participant-aligned fixture ⇒ scheme i IS participant i.
                assert_eq!(scheme.me(), Some(Participant::new(i as u32)));
                scheme
            })
            .collect();
        let verifier = SchnorrScheme::verifier(
            ops.participants.clone(),
            pubkeys,
            directory.clone(),
            ATTEMPTS,
