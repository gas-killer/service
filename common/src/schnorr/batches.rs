...
impl CompletionCollector {
    ...
    /// Validates and adds one member's partial (`sender` = the authenticated p2p identity
    /// the message arrived under, which is also the scheme's participant key).
    pub fn add(&mut self, sender: &OperatorKey, r_addr: Address, partial: Scalar) -> Added {
...
mod tests {
    ...
    fn completion_round_end_to_end() {
        let ops = crate::schnorr::scheme::test_support::operators(4, 200);
        let keys = &ops.keys;
        ...
        for (n, i) in members.iter().enumerate() {
            ...
            let sender = &ops.identities[*i as usize];
