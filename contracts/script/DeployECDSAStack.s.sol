...
contract DeployECDSAStack is Script {
    /// @notice Default id of the slashable operator set (mirrored by the router's
    ///         `AVS_OPSET_ID` metadata and the `GasKillerSlasher`'s constructor arg).
    ///         Override with the OPERATOR_SET_ID env var — the same one
    ///         `DeployGasKillerSlasher` and `EnrollOperatorSlashing` read, so the
    ...
    function run() external {
        ...
        ECDSAStakeRegistry registry = new ECDSAStakeRegistry(IDelegationManager(delegationManager));
        GasKillerServiceManager serviceManager = new GasKillerServiceManager(
            avsDirectory, address(registry), rewardsCoordinator, delegationManager, allocationManager, deployer
        );

        IECDSAStakeRegistryTypes.Quorum memory quorum;
        quorum.strategies = new IECDSAStakeRegistryTypes.StrategyParams[](1);
        quorum.strategies[0] =
            IECDSAStakeRegistryTypes.StrategyParams({strategy: IStrategy(strategy), multiplier: 10_000});
        ...
        if (allocationManager != address(0)) {
            serviceManager.updateAllocationManagerMetadataURI(vm.envOr("AVS_METADATA_URI", string("gas-killer-avs")));
            IStrategy[] memory operatorSetStrategies = new IStrategy[](1);
            operatorSetStrategies[0] = IStrategy(strategy);
            serviceManager.createOperatorSet(
                uint32(vm.envOr("OPERATOR_SET_ID", uint256(DEFAULT_OPERATOR_SET_ID))), operatorSetStrategies
            );
        }

        vm.stopBroadcast();

        string memory inner = vm.serializeAddress("addresses", "ecdsaStakeRegistry", address(registry));
        inner = vm.serializeAddress("addresses", "gasKillerServiceManager", address(serviceManager));
        string memory output = vm.serializeString("root", "addresses", inner);
        string memory path =
            string.concat(vm.projectRoot(), "/script/deployments/ecdsa-stack/", vm.toString(block.chainid), ".json");
