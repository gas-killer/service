...
import {SchnorrNonceRegistry} from "../src/SchnorrNonceRegistry.sol";
...
import {ISchnorrNonceRegistry} from "../src/interface/ISchnorrNonceRegistry.sol";
...
contract SchnorrNonceRegistryTest is Test {
    ...
    function setUp() public {
        vm.roll(1000);
        stakeRegistry = new SchnorrStakeRegistry(2, 3, address(this));
        deployCodeTo(
            "SchnorrNonceRegistry.sol:SchnorrNonceRegistry",
            abi.encode(ISchnorrStakeRegistry(address(stakeRegistry))),
            REGISTRY_ADDR
        );
    ...
    function test_keyMismatchAgainstStakeRegistry_rejected() public {
        ...
        MockStakeRegistry mock = new MockStakeRegistry();
        SchnorrNonceRegistry fresh = new SchnorrNonceRegistry(ISchnorrStakeRegistry(address(mock)));
    ...
    /// Mirrors `precommit::tests::batch_message_parity_vector` — identical inputs,
    /// identical constant. If the preimage layout changes, regenerate BOTH.
    function test_batchMessage_parityWithRust() public view {
        ...
        bytes32 message = registry.batchMessage(
            0xaAaAaAaaAaAaAaaAaAAAAAAAAaaaAaAaAaaAaaAa, 1, 1024, 2048, bytes32(uint256(0x1111111111111111111111111111111111111111111111111111111111111111))
        );
...
/// @dev Minimal stake registry mock for the KeyMismatch branch.
contract MockStakeRegistry {
    ...
    function operators(address id) external view returns (uint256, uint256, uint256, bool) {
        Record storage r = records[id];
        return (r.x, r.y, 0, r.registered);
    }

    function isValidSignature(bytes32, uint256, address, address[] calldata, uint256)
        external
        pure
        returns (bool)
    {
