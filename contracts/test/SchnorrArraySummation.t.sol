...
import {SchnorrArraySummationFactory} from
    "../src/examples/array-summation/SchnorrArraySummationFactory.sol";
import {ISchnorrGasKillerSDK} from "../src/interface/ISchnorrGasKillerSDK.sol";
...
contract SchnorrArraySummationTest is Test {
    /// @dev The ECDSA `IGasKillerSDK` interface id — the schnorr target must NOT report it,
    ///      or the router's ECDSA preflight would mistake the contract for an ECDSA target.
    bytes4 internal constant ECDSA_GAS_KILLER_INTERFACE_ID = 0xeb9ecb2e;
    ...
    function _digest(uint256 transitionIndex, bytes4 targetFn, bytes memory updates)
        internal
        view
        returns (bytes32)
    {
    ...
    function test_supportsInterface() public view {
        ...
        assertTrue(
            target.supportsInterface(type(ISchnorrGasKillerSDK).interfaceId),
            "ISchnorrGasKillerSDK not supported"
        );
        assertFalse(
            target.supportsInterface(ECDSA_GAS_KILLER_INTERFACE_ID),
            "ECDSA interface id must be unsupported"
        );
    ...
    function test_getMessageHash_parity() public view {
        ...
        bytes32 expected =
            sha256(abi.encode(uint256(0), address(target), SchnorrArraySummation.sum.selector, updates));
        assertEq(
