...
///         piece that makes aggregate-Schnorr signing non-interactive (see
///         `docs/schnorr-nonce-registry.md`). Operators commit a Merkle root per batch of
///         nonce *points*; the full points travel p2p and peers verify them against these
///         roots. The chain never stores, opens, or verifies a nonce point.
///
...
contract SchnorrNonceRegistry is ISchnorrNonceRegistry {
    ...
    ISchnorrStakeRegistry public immutable stakeRegistry;
    ...
    constructor(ISchnorrStakeRegistry _stakeRegistry) {
    ...
    function batchMessage(address operatorId, uint64 batchIndex, uint64 startSlot, uint64 count, bytes32 root)
        ...
        returns (bytes32)
    {
        return keccak256(abi.encodePacked(BATCH_TAG, block.chainid, address(this), operatorId, batchIndex, startSlot, count, root));
    }
