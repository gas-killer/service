...
POST /bridge/ask {"model":"qwen"|"qwen35","prompt_ids":[...],"max_new":8}
  -> {"ask_id": "..."} immediately; work continues in a thread:
     ...
     2. When the answer lands, build fulfil(promptIds,maxNew,answerIds,root)
        ...
        POST /tasks, then poll GET /tasks/<task_id> until the round renders a
        payload. The operator gate re-verifies the segment commit chain before
        signing.

The router does not broadcast: a completed round yields a ready-to-sign
transaction that the holder of a funded key submits. The bridge has no signing
key, so it reports the payload and stops there.
GET /bridge/ask/<id> -> status JSON (inferring -> settling -> ready | error)
...
# A settlement round is one aggregation over the operator committee. The ceiling is
# generous because the round queues behind any other task the router is aggregating.
SETTLE_TIMEOUT = float(os.environ.get("SETTLE_TIMEOUT_SECONDS", "900"))
SETTLE_POLL_SECONDS = float(os.environ.get("SETTLE_POLL_SECONDS", "5"))
AUTH = {"Authorization": f"Bearer {GK_KEY}"}
...
def await_payload(task_id, timeout=None, now=time.time, sleep=time.sleep):
    """Poll GET /tasks/<task_id> until the settlement round renders a payload.

    Returns the payload dict ({to, data, value, chain_id, estimated_gas,
    valid_until_block}) that a funded key would sign and broadcast. Raises on a
    terminal failure, on an unrecognised status, and on timeout, so an ask that did
    not settle carries the reason rather than reporting a success that never happened.

    `now`/`sleep` are injectable for tests; production uses the wall clock.
    """
    deadline = now() + (SETTLE_TIMEOUT if timeout is None else timeout)
    while True:
        task = http_json(f"{API}/tasks/{task_id}", headers=AUTH, timeout=30)
        status = task.get("status")
        if status == "ready":
            payload = task.get("payload")
            if not payload:
                raise RuntimeError(f"task {task_id} is ready but carries no payload")
            return payload
        if status in ("failed", "expired"):
            raise RuntimeError(
                f"task {task_id} {status}: {task.get('error') or 'no reason given'}")
        if status not in ("queued", "processing"):
            raise RuntimeError(f"task {task_id} reported unknown status {status!r}")
        if now() >= deadline:
            raise TimeoutError(f"task {task_id} still {status} after the settle timeout")
...
def run_ask(ask_id, model, prompt_ids, max_new):
    ...
    try:
        ...
        accepted = http_json(f"{API}/tasks", body, headers=AUTH, timeout=60)
        task_id = accepted["task_id"]
        st.update(task_id=task_id, transition_index=ti)
        payload = await_payload(task_id)
        st.update(state="ready", payload=payload,
