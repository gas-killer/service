...
POST /bridge/ask {"model":"qwen"|"qwen35","prompt_ids":[...],"max_new":8}
  -> {"ask_id"} immediately; a worker thread then:
     ...
     2. builds fulfil(promptIds,maxNew,answerIds,pipelineRoot) calldata and
        submits ONE settlement round via the router's public POST /tasks (the
        bridge holds the gk_ API key), then polls GET /tasks/<task_id> until the
        round renders a payload; the operator gate re-verifies the segment
        commit chain before signing.
GET /bridge/ask/<id>  -> queued | inferring | settling | ready | error
GET /bridge/healthz
```

One inference per model at a time (429 while busy). Stdlib-only Python.

The router does not broadcast the settlement. A completed round leaves a
ready-to-sign transaction (`to`, `data`, `value`, `chain_id`, `estimated_gas`,
`valid_until_block`), which the bridge reports as `payload` on the terminal
`ready` state. Landing the answer on-chain needs a funded key, which the bridge
does not hold — so the final submission is out of band.

Run the unit tests with `python3 -m unittest discover -s bridge`.
