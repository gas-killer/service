...
impl<P: Provider<Ethereum> + Clone + Send + Sync + 'static> GasKillerHandler<P> {
    ...
    /// Schnorr twin of [`Self::handle_verification`]: same metrics envelope, the
    /// proof arguments swap `(operators, signatures)` for the aggregate
    /// `(s, Raddr)` and the strictly ascending `non_signers`.
    #[allow(clippy::too_many_arguments)]
