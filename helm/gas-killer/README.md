...
| `global.guestPrograms` | UNBOUNDED_V3 guest programs served behind the gkvm precompile. List of `{name, url, programHash}`; a `fetch-guest-vm` initContainer on the router and every node downloads each to `<guestMountPath>/programs/<name>.elf` and exports the `GK_GUEST_PROGRAM[_N]` / `GK_GUEST_PROGRAM_HASH[_N]` env pair (bare names first, then `_1`, `_2`, … in list order). The service keccaks every ELF against its `programHash` at startup and refuses to boot on a mismatch. Requires `simExecutor: local` (the chart and the binary both refuse guest programs under `rpc`). Pinned-env config: a single global list, so router and nodes install the same set; an operator without a program abstains on its tasks. | `[]` |
| `global.guestArtifacts` | Merkle v3 artifact bundles the guest programs read (e.g. model weights). List of `{name, baseUrl, files, artifactRoot}`; `files` is the bundle in MANIFEST ORDER, downloaded to `<guestMountPath>/artifacts/<name>/` and mounted as one `GK_GUEST_ARTIFACT[_N]` slot (paths joined with `:`) + `GK_GUEST_ARTIFACT_ROOT[_N]`, verified at startup and served front to back. | `[]` |
| `global.guestVmConsumers` | The `requiresGuestVm` consumer registry (`GK_GUEST_VM_CONSUMERS`). List of `{consumer, programHash}` (repeat the consumer for several programs). A registered consumer is analyzed only under `simExecutor: local` with that program installed; any other operator abstains instead of signing the `GkVmUnavailable` revert as a competing transition — so set it whatever the executor. | `[]` |
| `global.guestDownloaderImage` | Image for the `fetch-guest-vm` initContainer (needs `curl`). | `curlimages/curl:8.10.1` |
| `global.guestMountPath` | Where `programs/` and `artifacts/` land inside the pods. | `/guest` |
| `overlay.extraModels` | Additional pinned models mounted SIMULTANEOUSLY with the primary overlay (multi-overlay, gas-analyzer#172). List of `{name, artifactBaseUrl, weightsFile, tokenizerFile, manifest}`; each downloads into `<overlay.mountPath>/<name>/` and exports the numbered `GK_OVERLAY_*_N` env triplet (N starts at 2, in list order). Requires `simExecutor: local` with mmap mode; manifests must be distinct (chunk addresses are derived per-manifest, so the models' address sets are disjoint and each consumer finds exactly its own model's chunks). Pinned-env config: router and all nodes must mount the same model set. | `[]` |
